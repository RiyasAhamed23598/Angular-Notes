...
import {concat, delay, from, interval, map, take} from "rxjs";
...
describe('Marble testing in RxJs', () => {
  ...
  it('should let you identify subscription points', () => {
    testScheduler.run(helpers => {
      ...
      const sourceOneExpectedSubscription = '^------!'
      const sourceTwoExpectedSubscription = '-------^------!'

      expectObservable(final$).toBe(expected);
      expectSubscriptions(source$.subscriptions).toBe(sourceOneExpectedSubscription);
      expectSubscriptions(source2$.subscriptions).toBe(sourceTwoExpectedSubscription);
    })
  });
  ...
  it('should let you identify subscription points', () => {
    testScheduler.run(helpers => {
      ...
      const source$ = hot('-a-b-^-c').pipe(take(1));
  ...
  it('should let you test snapshots of streams that do not complete', () => {
    testScheduler.run(helpers => {
      ...
      const source$ = interval(1000).pipe(map((val) => {
        return `${val + 1}sec`;
      }))
